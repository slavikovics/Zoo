...
(
    ...
    CONSTRAINT Check_Employees_Age CHECK (EXTRACT(YEAR FROM AGE(BirthDate)) >= 18),
    CONSTRAINT Check_Employees_BirthDate CHECK (BirthDate <= CURRENT_DATE)
);
...
(
    ...
    CONSTRAINT UQ_EmployeeSpouse_EmployeeId UNIQUE (EmployeeId),
    CONSTRAINT UQ_EmployeeSpouse_SpouseId UNIQUE (SpouseId),
    CONSTRAINT CHK_EmployeeSpouse_NoSelfReference CHECK (EmployeeId != SpouseId)
);
...
-- 12. Процедура для добавления связи между супругами
...
BEGIN
    ...
    IF p_employee_id = p_spouse_id THEN
        RAISE EXCEPTION 'Нельзя добавить сотрудника % в качестве супруга самому себе', p_employee_id;
    END IF;

    -- Проеряем, чтобы сотрудники не состояли в браке
    IF EXISTS (SELECT 1 FROM EmployeeSpouse WHERE EmployeeId = p_employee_id OR SpouseId = p_employee_id) THEN
        RAISE EXCEPTION 'Сотрудник с ID % уже состоит в браке', p_employee_id;
    END IF;

    IF EXISTS (SELECT 1 FROM EmployeeSpouse WHERE EmployeeId = p_spouse_id OR SpouseId = p_spouse_id) THEN
        RAISE EXCEPTION 'Сотрудник с ID % уже состоит в браке', p_spouse_id;
    END IF;
...
-- 13. Процедура для удаления супружеской связи между сотрудниками
CREATE OR REPLACE PROCEDURE RemoveEmployeeSpouse(
    p_employee_id INT
)
    LANGUAGE plpgsql
AS
...
DECLARE
    v_spouse_id INT;
BEGIN
    -- Находим ID супруга (может быть как в EmployeeId, так и в SpouseId)
    SELECT CASE
    ...
    ELSE
        RAISE NOTICE 'Супружеская связь для сотрудника % не найдена', p_employee_id;
    END IF;
END;
$$;

-- 14. Функция для получения SpouseId для Employee
...
-- 15. Триггерная функция для проверки согласованности статуса при обновлении сотрудника (при создании статус может быть любым)
CREATE OR REPLACE FUNCTION CheckMaritalStatusConsistency()
    RETURNS TRIGGER
    LANGUAGE plpgsql
AS
$$
BEGIN
    -- Если сотрудник есть в EmployeeSpouse, он должен иметь статус 'Married'
    IF NEW.MaritalStatus != 'Married' AND
       EXISTS (SELECT 1 FROM EmployeeSpouse WHERE EmployeeId = NEW.Id OR SpouseId = NEW.Id) THEN
        RAISE EXCEPTION 'Сотрудник % состоит в браке, статус должен быть "Married"', NEW.Id;
    END IF;
    RETURN NEW;
END;
$$;
...
$$
BEGIN
    IF EXISTS(SELECT 1 FROM Employees WHERE Id = OLD.EmployeeId) AND
       EXISTS(SELECT 1 FROM Employees WHERE Id = OLD.SpouseId) THEN
        UPDATE Employees
